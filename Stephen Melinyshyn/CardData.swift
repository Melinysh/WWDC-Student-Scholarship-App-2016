...
class CardData: NSObject {
	...
	let images = [ "me1", "me2" , "tapz", "rez", "marketmesh", "meshwurk", "codefest", "wwdc","wellaware",  /*"me2"*/ "wattpad"]
	...
	let descriptions = [
		"Welcome to my WWDC App Submission! I'm a 19 year old Canadian with a passion for computer science. Although I've just completed my first year in Software Engineering at the University of Waterloo, my own curiosity and persistence has driven me to explore and experiment with code over the past few years. Using what I've learned over the past several years, I've taken the last 2 weeks building this app to inform you about what exciting things I've done on Apple's platform, what I'm working on, and what I will be doing in the future. To dismiss this card, just swipe it away!",
		...
		"After surviving my midterm week (five midterm exams in five days), I took the weekend off with some classmates to participate in the City of Waterloo's first open data hackathon, CodeFest. We developed an elegant augmented reality app called NearbEYE. While using NearbEYE, a user can point their camera in the direction of interest and NearbEYE will beautifully display relevant open data about points of interest in that direction. To be able to do this, I devised a custom algorithm that took in compass and geolocation data and fetched relevant data from our Core Data store. We utilized Core Location for our algorithm, AVCapture APIs for live camera input, Core Data for storing all point of interest data, and Mapkit to provide the walking route to points of interest. The app integrated 8 unique datasets, more than double of any other team. NearbEYE won third place for its \"impressive technical feats\" and I believe pushed the boundaries of the platform for the time period we had.",
		
		"Recently, I've been looking for a new challenge. The WWDC Student Scholarship program is a fantastic opportunity to exercise and learn skills. From the start, I was driven to create a simple, playful user interface. Beautiful stacked cards that dropped down in front of the user was a key idea for me. Behind the scenes, it took a considerable amount of work, dynamically generating the cards and the angles at which they fall (if you reset the card stack, you'll see the cards fall rotated differently each time). It also works on both iPad and iPhone through the use of Autolayout and smart design. In additon, UIDynamics provide the playful bounce to the cards. The app is entirely written by me; no third party code! It also borrows a few ideas I first developed in Rez Reader, but enhanced significantly. I consider this application a culmination of my best iOS programming to date.",
		
		"WellAware is a current project that I'm parterning with one other person to creare. WellAware is tool to locate and report wellwater conditions in the southern Ontario region. Based off of extensive research and data collected by my partner, we are able to determine areas at risk of water contamination. For this project, I am the sole developer of the iOS application while my partner is responsible for visual design, and data cultivation. For the app, I'm leveraging Core Data for storing all risk data and for geospatial lookups, MapKit for setting the lookup location and for proving directions to the nearest wellwater testing drop off location. WellAware is still under construction, but we plan to have it done  by the end of the summer with interest from third parties.",
